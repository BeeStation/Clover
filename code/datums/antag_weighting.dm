...
/datum/antagWeighter
	...
	/**
	 * Queries the cloverfield API for hisorical antag rounds for a single target
	...
	*/
	proc/completeHistory(ckey = "")
		...
		if (!config.cloverfield_api_token)
			throw EXCEPTION("You must have the cloverfield API token to use this command!")
	...
	/**
	 * Queries the cloverfield API for hisorical antag rounds for the pool of minds given
	 *
	 * @param string role Name of the antag role we're picking for (e.g. traitor, spy_thief)
	 * @param list history List of historical antag data returned by the cloverfield API
	...
	proc/choose(list/pool = list(), role = "", amount = 0, recordChosen = 0)
		...
		var/list/response
		if (config.cloverfield_api_token && apiHandler.enabled)
			//YO API WADDUP
			try
